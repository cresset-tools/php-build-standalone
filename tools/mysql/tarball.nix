# MySQL tarball derivation. Mirrors tools/mariadb/tarball.nix but for the
# `tool` section: emits one tarball carrying mysqld, the client tools,
# libmysqlclient, and the plugin set. Bundled C-libs (OpenSSL, zlib,
# ncurses) ride separately as closure entries in the manifest — clients
# resolve them through the shared `$BOUGIE_HOME/store/` pool.
#
# Produces under $out:
#   mysql-<ver>-<triple>.tar.zst   redistributable artifact
#   mysql-<ver>-<triple>.json      fat manifest (DISTRIBUTION.md shape)
#
...
# routes the manifest into versions/<V>/targets/<T>/sections/tool/. Two
# MySQL lines (8.0, 8.4) produce distinct tags and coexist under one
# sections/tool/mysql section.
{ pkgs, tree, sources
, target ? if pkgs.stdenv.isDarwin then "aarch64-apple-darwin" else "x86_64-unknown-linux-gnu"
, mysqlVersion ? "0.0.0-unknown"
, nixpkgsRev
...
let
  ...
  # which is now just mysql itself. The bundled C-libs are carried
  # by closure[] instead and audited there.
  bundledLibraries = { mysql = mysqlVersion; };
  ...
  flavor = "default";
  tag = "mysql-${mysqlVersion}-${target}-${flavor}";
  ...
  metadata = {
    ...
    binaries = [ "mysqld" "mysql" "mysqldump" "mysqladmin" "mysqlbinlog" ];
  ...
  metadataFile = pkgs.writeText "mysql.json.in" (builtins.toJSON metadata);
...
pkgs.stdenvNoCC.mkDerivation {
  ...
  installPhase = ''
    ...
    mkdir -p "$out"
    base="mysql-${mysqlVersion}-${target}"
    ...
    # against the shared $BOUGIE_HOME/store/ pool.
